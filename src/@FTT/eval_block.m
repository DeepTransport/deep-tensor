...
if dir > 0
    ...
    for j = 1:min(k,ndims(obj))
        nj  = size(obj.cores{j}, 2);
        rjm = size(obj.cores{j}, 1);
        %
        if j < ndims(obj) || (size(obj.cores{j}, 3) > 1 && size(obj.cores{j}, 4) == 1)
    ...
    for i = 1:min(k,ndims(obj))
        ...
        T   = reshape(permute(reshape( eval_radon(obj.oneds{j},tmp,x(xind(i),:)), nx, rj, []), [2,1,3]), rj*nx, []);
        % how to speed up this part?
