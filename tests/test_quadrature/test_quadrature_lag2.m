...
tic;
[y1,grad] = quad_Lagrange2_deri(poly, theta, x);
toc
[quadx,quadw] = quadcc_piecewise(poly, 33, x);
tic;
...
y2 = y1;
for i = 1:length(x)
    y2(i) = quad(@(z) g(poly, theta(:,i), z) , poly.domain(1), x(i), 1E-10 );
end
norm(y2 - y1)/norm(y2)
norm(y3 - y2)/norm(y3)
...
for j = 1:poly.num_nodes
    ...
    y1p(j,:) = quad_Lagrange2_deri(poly, thetap, x);
    y1m(j,:) = quad_Lagrange2_deri(poly, thetam, x);
end
gd = (y1p-y1m)/(2*tol);
plot(gd(:), grad(:), '.')
norm(gd-grad,'fro')
