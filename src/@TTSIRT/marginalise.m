...
if obj.int_dir > 0
    ...
    for k = d:-1:1
        ...
        obj.ys{k} = reshape( reshape(obj.approx.data.cores{k}, rkm*nx, rk)*Ligeqk, rkm, nx, rk);
        
        %disp(size(obj.approx.data.cores{k}))
        %disp(size(ys{k}))
        
        B = reshape( mass_r(obj.approx.base.oneds{k}, reshape(permute(obj.ys{k},[2,3,1]),nx,[])), [],rkm);
        [~, R] = qr(B,0);
    ...
    for k = 1:d
        ...
        B = reshape(mass_r(obj.approx.base.oneds{k},reshape(permute(obj.ys{k},[2,1,3]),nx,[])), [],rk);
        [~,Rileqk] = qr(B,0);
