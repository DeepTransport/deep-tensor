...
% Evaluate the gradient of the TTFunRef
%   [g,f] = GRAD(tt, x)
%
...
nx  = size(x, 2);
fxl = cell(ndims(obj.base),1);
fxr = cell(ndims(obj.base),1);
    
for j = 1:ndims(obj.base)
    ...
    % rjm nx rj
    if j == 1
        fxl{j} = reshape(permute(reshape( eval_radon(obj.base.oneds{j},tmp,x(j,:)), nx, rjm, []), [2,1,3]), rjm*nx, []);
    else
...
for j = ndims(obj.base):-1:1
    ...
    % rj nx rjm
    if j == ndims(obj.base)
        fxr{j} = reshape(permute(reshape( eval_radon(obj.base.oneds{j},tmp,x(j,:)), nx, rj, []), [3,1,2]), rjm, []);
    else
        T = reshape(permute(reshape( eval_radon(obj.base.oneds{j},tmp,x(j,:)), nx, rj, []), [2,1,3]), rj*nx, []);
        % how to speed up this part?
...
for j = 1:ndims(obj.base)
    ...
    tmp = reshape(permute(obj.data.cores{j}, [2,1,3]), nj, []);
    % (rjm nx) by rj
    D = reshape(permute(reshape( eval_radon_deri(obj.base.oneds{j},tmp,x(j,:)), nx, rjm, []), [2,1,3]), rjm*nx, []);
    if j == 1
        gx(j,:) = sum(D'.*fxr{j+1}, 1);
    elseif j == ndims(obj.base)
        jj  = reshape(reshape(1:rjm*nx, rjm, nx)', [], 1);
        ii  = repmat((1:nx)', 1, rjm);
        % nx by (rjm nx)
