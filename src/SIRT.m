classdef SIRT < AbstractIRT
    ...
    %%%%%%%%%%%%%%%%%
    %
    % see also TTSIRT and SparseSIRT
    ...
    methods
        function obj = SIRT(potential, arg, varargin)
            [potential,arg,opt,var,tau,base] = parse_input(obj,potential,arg,varargin{:});
            %
            func = @(z) SIRT.potential_to_density(base, potential, z);
            ...
            for i = 1:length(obj.approx.base.oneds)
                obj.oned_cdfs{i} = CDFconstructor(obj.approx.base.oneds{i}, obj.approx.opt.cdf_tol);
            end
            obj.tau = tau;
            obj = marginalise(obj);
        end
