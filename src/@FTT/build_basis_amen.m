...
%
[B,A,r] = FTT.local_truncate(loc_err_tol, 1, max_rank, oned, F);
%
% compute residual
...
[B,A,r]     = FTT.local_truncate(loc_err_tol, 1, max_rank, oned, [F,Fu]);
rnew        = size(B,2);
%
[indf,core,interp_atx] = point_selection(oned, int_method, B);
...
%}
%
interp_x    = FTT.local_index(oned, dir, interp_xold, indf);
%
Qr      = FTT.local_truncate(loc_err_tol*eps, 1, kick_rank, oned, Fr);
% rold is the leading dimension in the point selection
indr    = point_selection(oned, int_method, Qr);
res_x   = FTT.local_index(oned, dir, res_xold, indr);
%
if dir > 0
    % from left >k is integrated
