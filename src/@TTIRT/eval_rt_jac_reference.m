...
if obj.int_dir > 0 % from left to right
    ...
    for k = 1:d
        block_ftt{k} = TTSIRT.eval_oned_core_213(obj.approx.base.oneds{k}, obj.approx.data.cores{k}, r(k,:));
        block_mar{k} = TTSIRT.eval_oned_core_213(obj.approx.base.oneds{k}, obj.ys{k}, r(k,:));
        %
        rkm  = size(obj.approx.data.cores{k}, 1);
        T{k} = reshape(TTSIRT.eval_oned_core_213(obj.approx.base.oneds{k}, obj.ys{k}, obj.oned_cdfs{k}.nodes(:)), rkm, []);
        %
        block_ftt_d{k} = TTSIRT.eval_oned_core_213_deri(obj.approx.base.oneds{k}, obj.approx.data.cores{k}, r(k,:));
        %
    ...
    for j = 1:d
        ...
        if j < d % skip the (d,d) element
            ...
            mrl = TTSIRT.eval_oned_core_213_deri(obj.approx.base.oneds{j}, obj.ys{j}, r(j,:));
            if j > 1
                rjm = size(obj.approx.data.cores{j}, 1);
            ...
            for k = (j+1):d 
                ...
                % the first term
                if obj.approx.base.oneds{k}.constant_weight
                    tmp = eval_measure(obj.approx.base.oneds{k}, obj.oned_cdfs{k}.nodes);
                    pk = pk.*tmp(:);
                end
    ...
    block_ftt_d = cell(d,1);
    for k = 1:d
        block_ftt{k} = TTSIRT.eval_oned_core_231(obj.approx.base.oneds{k}, obj.approx.data.cores{k}, r(k,:));
        block_mar{k} = TTSIRT.eval_oned_core_231(obj.approx.base.oneds{k}, obj.ys{k}, r(k,:));
        rk   = size(obj.approx.data.cores{k}, 3);
        T{k} = reshape(TTSIRT.eval_oned_core_213(obj.approx.base.oneds{k}, obj.ys{k}, obj.oned_cdfs{k}.nodes(:)), [], rk);
        block_ftt_d{k} = TTSIRT.eval_oned_core_231_deri(obj.approx.base.oneds{k}, obj.approx.data.cores{k}, r(k,:));
        %
        logw = eval_log_measure(obj.approx.base.oneds{k}, r(k,:));
        mlogw{k} = -logw;
    end
    ...
    for j = d:-1:1
        ...
        if j > 1 % skip the (1,1) element
            ...
            drg = block_ftt_d{j}';
            % derivative of the FTT, 2nd term, for the d(j+1)/dj term
            mrg = TTSIRT.eval_oned_core_231_deri(obj.approx.base.oneds{j}, obj.ys{j}, r(j,:))';
            if j < d
                rj  = size(obj.approx.data.cores{j}, 3);
                ii  = reshape(1:rj*n, [], 1);
